<?php
/**
 * Configuración de correos para Outlook - SOLUCIÓN DEFINITIVA
...
function enviarNotificacionInsignia($destinatario_email, $datos_insignia) {
    ...
    if (file_exists('src/PHPMailer.php')) {
        return enviarConPHPMailerOutlook($destinatario_email, $datos_insignia);
    } else {
        return enviarConMailNativo($destinatario_email, $datos_insignia);
    }
}

function enviarConPHPMailerOutlook($destinatario_email, $datos_insignia) {
    ...
    try {
        ...
        // Enviar el correo
        $mail->send();
        error_log("Correo enviado exitosamente con PHPMailer Outlook a: " . $destinatario_email);
        return true;
        
    } catch (Exception $e) {
        error_log("Error PHPMailer Outlook al enviar correo a " . $destinatario_email . ": " . $e->getMessage());
        return false;
    }
}

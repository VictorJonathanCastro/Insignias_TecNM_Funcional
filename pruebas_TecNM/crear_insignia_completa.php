...
echo "<h2>🔧 Crear insignia completa para validación</h2>";
...
if ($result && $result->num_rows > 0) {
    ...
    echo "<li><a href='ver_insignia_completa.php?insignia=" . urlencode($codigo_buscar) . "' target='_blank'>Ver insignia completa</a></li>";
    echo "</ul>";
    ...
    try {
        ...
        if (!$result || $result->num_rows == 0) {
            // Crear destinatario
            $sql_dest = "INSERT INTO destinatario (Id_Centro, Nombre_Completo, Nombre, Apellido_Paterno, Apellido_Materno, Correo, Rol, ITCentro) VALUES (1, 'Victor Jonathan Castro Secundino', 'Victor Jonathan', 'Castro', 'Secundino', 'victor.castro@tecnm.mx', 'Estudiante', 1)";
        ...
        // Verificar insignia
        $result = $conexion->query("SELECT id FROM insignias WHERE id = $insignia_id");
        if (!$result || $result->num_rows == 0) {
            // Crear insignia
            $sql_ins = "INSERT INTO insignias (Tipo_Insignia, Propone_Insignia, Programa, Descripcion, Criterio, Fecha_Creacion, Fecha_Autorizacion, Nombre_gen_ins, Estatus, Archivo_Visual) VALUES (1, 1, 'Ingeniería en Sistemas', 'Esta insignia reconoce la participación destacada en actividades de responsabilidad social por parte del estudiante.', 'Completar actividades de responsabilidad social', '2025-01-15', '2025-01-15', 'TecNM-ITSM', 1, 'insignia_Responsabilidad Social.png')";
...
echo "<h3>🎯 Resumen:</h3>";
echo "<p>Si la insignia se crea correctamente, el sistema de validación debería funcionar con el código <strong>$codigo_buscar</strong>.</p>";
?>

...
try {
    ...
    if ($stmt) {
        ...
        if ($row = $result->fetch_assoc()) {
            // Determinar la imagen dinámica según la subcategoría
            $archivo_imagen = $row['archivo_imagen'];
            
            if (!empty($archivo_imagen)) {
                // Buscar primero en imagenes/ (con 's')
                $imagen_path = 'imagenes/' . $archivo_imagen;
                
                // Si no existe, buscar en imagen/Insignias/
                if (!file_exists($imagen_path)) {
                    $imagen_path = 'imagen/Insignias/' . $archivo_imagen;
                    
                    // Si tampoco existe ahí, buscar en imagen/ principal
                    if (!file_exists($imagen_path)) {
...
$current_url = $base_url . '/facebook_imagen.php?codigo=' . urlencode($codigo_insignia);
?>
<!DOCTYPE html>
<html lang="es">
<head>
...
<body>
    <div class="container">
        ...
        <div class="click-instruction">
            ...
            <p>⏰ Serás redirigido automáticamente en 2 minutos</p>
            <p>🛑 Haz clic en "Ver Certificado Completo" para ir inmediatamente</p>
        </div>
        ...
        <div class="facebook-success">
            <h4>✅ Facebook está funcionando</h4>
            <p>Facebook ya está mostrando el preview del enlace. Si no aparece la imagen, usa el Facebook Debugger para forzar la actualización.</p>
        </div>
        
        <div class="debug-info">
            <h4>🔧 Información de depuración:</h4>
            <p><strong>URL actual:</strong> <?php echo $current_url; ?></p>
            <p><strong>URL de la imagen:</strong> <?php echo $image_url; ?></p>
            <p><strong>Es Facebook bot:</strong> <?php echo $is_facebook_bot ? 'Sí' : 'No'; ?></p>
    ...
    <script>
        ...
        if (!<?php echo $is_facebook_bot ? 'true' : 'false'; ?>) {
            // Redirigir automáticamente al certificado después de 2 minutos
            setTimeout(function() {
                window.location.href = '<?php echo $validation_url; ?>';
            }, 120000);
        }
        ...
        // Redirigir inmediatamente si es un clic desde Facebook
